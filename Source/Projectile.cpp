...
void Projectile::UpdateTransform()
{
	...
	//前方向ベクトルを算出　←（Front のベクトルのこと）
	Front = DirectX::XMLoadFloat3(&direction);//このままじゃ使えない。なので↓
	Front = DirectX::XMVector3Normalize(Front);//一定の長さにして適正化（これで使えるようになる）
	...
	//右ベクトルを算出
	Right = DirectX::XMVector3Cross(Up, Front);//←Up と Front に両方とも垂直な方向（Right）を外積で求める
