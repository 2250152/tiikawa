...
void ProjectileHoming::Update(float elapsedTime)
{
	...
	if (lifeTimer <= 0.0f)//  パソコンは誤差がでるので==ではなく、<= がいい。
	{
		Destroy();
	}
	...
	//移動
	{
		...
		position.x += direction.x * moveSpeed;  //directionは方向
	...
	//旋回
	{
		...
		DirectX::XMVECTOR Vec = DirectX::XMVectorSubtract(Target, Position);//２点間のベクトル値に変換らしい。ほんまか分からんから調べて。
		...
		if (lengthSq > 0.00001f)
		{
			...
			if (fabsf(rot) > 0.0001f)
			{
				...
				Transform = DirectX::XMMatrixMultiply(Transform, Rotation); //←XMMatrixMultiplyは掛け算
	                          ...
	                          const DirectX::XMFLOAT3& target)
{
	...
	UpdateTransform(); //Transform は日本語で "変換する" 
}